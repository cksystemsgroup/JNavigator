...
public interface JAviatorPhysicalModel
{
   ...
   * @param parameters the parameters to set (depends entirely on the model implementation)
   */
 ...
 /**
   ...
   * @param motor new motor speeds
   */
  public void setMotorSignals ( MotorSignals actuator );
  
  /**
   * Specify the current navigation data.  A proper simulation should ignore this information,
   *   since the control should have used it to generate proper motor values and the physical
   *   model should respond only to those.  However, some degenerate models used for testing
   *   may want to "cheat" by knowing the navigation data.
   * @param navigation the latest navigation data
   */
