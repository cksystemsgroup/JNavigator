...
import at.uni_salzburg.cs.ckgroup.communication.DataTransferObjectLogger;
...
public class JControlPosOnlyMain {
...
//	public static final String PROP_TCP_SERVER_PREFIX = "tcp.server.";
	...
	private JControlPosOnly jcontrol;
...
//	private TcpServer tcpServer;
	...
	private JControlPosOnlyMain (Properties props) throws InstantiationException, IOException {
		...
		jcontrol = (JControlPosOnly) ObjectFactory.getInstance ().instantiateObject (PROP_JCONTROL_PREFIX, JControlPosOnly.class, props);
		...
		logger = new DataTransferObjectLogger ();
		dispatcher.addDataTransferObjectListener (logger, IDataTransferObject.class);
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length >= 1) {
				...
				props = PropertyUtils.loadProperties ("at/uni_salzburg/cs/ckgroup/control/jcontrol-pos-only.properties");
			}
			JControlPosOnlyMain me = new JControlPosOnlyMain (props);
			me.run();
		} catch (Exception e) {
			e.printStackTrace();
		}
